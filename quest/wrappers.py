...
class SetState:
    ...
    async def _async_init(self, name: str, initial_value, identity):
        self.name = name
        self.identity = identity
        await _find_workflow().create_state(name, initial_value, identity)
        return self

    async def __call__(self, value):
        # Make invoke state also act like an event
        return await _find_workflow().set_state(self.name, value, self.identity)
...
async def state(name, initial_value=None, identity=None) -> SetState:
    ...
    return await SetState()._async_init(name, initial_value, identity)
...
class Queue:
    ...
    async def _async_init(self, name: str, identity: str | None) -> 'Queue':
        self.name = name
        self.identity = identity
        await _find_workflow().create_queue(name, identity)
        return self
