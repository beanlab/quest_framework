...
class Server:
    def __init__(self, manager: Historian, host: str, port: int, authorizer: Callable[[dict[str, str]], bool]):
        """
    ...
    async def __aenter__(self):
        ...
        history_logger.info(f'Server started at ws://{self._host}:{self._port}')
        return self
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        history_logger.info(f'Server at ws://{self._host}:{self._port} stopped')
    ...
    async def handler(self, ws: WebSocketServerProtocol, path: str):
        ...
        history_logger.info(f'New connection: {path}')
        if path == '/call':
