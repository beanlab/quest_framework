...
from .history import History, _Wrapper, SUSPENDED
...
class BookFactory(Protocol):
    def __call__(self, workflow_id: str) -> Book: ...
...
historian = ContextVar('historian')
...
class Historian:
    ...
    def __init__(self, namespace: str, storage: BlobStorage, create_book: BookFactory,
    ...
    async def __aenter__(self) -> 'Historian':
        ...
        for wid, data in self._workflow_data.items():
            ...
            self._start_soon(wtype, wid, args, kwargs, delete_on_finish=delete_on_finish)
    ...
    def _signal_handler(self, sig, frame):
        history_logger.debug(f'Caught KeyboardInterrupt: {sig}')
        for wid, history in self._workflows.items():
    ...
    def _start_soon(self,
        ...
        book = self.create_book(workflow_id)
        history: History = History(workflow_id, workflow_function, book, serializer=self._serializer)
        self._workflows[workflow_id] = history
    ...
    def start_soon(self, workflow_type: str, workflow_id: str, *workflow_args, delete_on_finish: bool = True,
                   **workflow_kwargs):
        ...
        self._start_soon(workflow_type, workflow_id, workflow_args, workflow_kwargs,
                         delete_on_finish=delete_on_finish)
    ...
    async def get_result(self, workflow_id: str, delete: bool = False):
        if workflow_id in self._workflow_tasks:
...
def find_historian() -> Historian:
    if (h := historian.get()) is not None:
        return h
