...
from .history_types import ConfigurationRecord, VersionRecord, StepStartRecord, StepEndRecord, \
...
from .utils import history_logger, task_name_getter
...
def _prune(step_id: str, history: "Book"):
    """
    Remove substep work
...
class History:
    ...
    def _reset_replay(self):
        history_logger.debug('Resetting replay')
    ...
    async def _replay_complete(self):
        ...
        history_logger.debug(f'{self.workflow_id} -- Replay Complete --')
            ...
            if record['task_id'] != task_id:
                if (gate := self._record_gates[_get_id(record)]).done():
                    if gate.exception() is not None:
                        history_logger.debug(f'{task_id} found {record} errored: {gate.exception()}')
                    else:
                        history_logger.debug(f'{task_id} found {record} completed')
                else:
                    history_logger.debug(f'{task_id} waiting on {record}')
                ...
                def complete(r, exc_type, exc_val, exc_tb):
                    if exc_type is not None:
                        ...
                        history_logger.debug(f'Noting that record {r} raised: \n{exc_info}')
                    # Note:
                    ...
                    history_logger.debug(f'{task_id} completing {r}')
                    self._record_gates[_get_id(r)].set_result(None)

                # noinspection PyUnboundLocalVariable
                history_logger.debug(f'{self._get_task_name()} replaying {record}')
                yield self._NextRecord(record, complete)

        history_logger.debug(f'Replay for {self._get_task_name()} complete')
        task_replay.set()
        await self._replay_complete()

    async def _external_handler(self):
        try:
            history_logger.debug(f'External event handler {self._get_task_name()} starting')
            ...
            history_logger.debug(f'External event handler {self._get_task_name()} completed')
        except Exception:
            history_logger.exception('Error in _external_handler')
            raise
    ...
    async def _run_configuration(self, config_record: ConfigurationRecord):
        ...
        history_logger.debug(f'Running configuration: {get_function_name(config_function)}(*{args}, **{kwargs})')
    ...
    def _replay_version(self, record: VersionRecord):
        history_logger.debug(f'{self._get_task_name()} setting version {record["step_id"]} = "{record["version"]}"')
    ...
    async def _after_version(self, module_name, func_name, version_name, version):
        ...
        if not found:
            history_logger.error(f'{self._get_task_name()} did not find version {version_name}=={version}')
    ...
    async def handle_step(self, func_name, func: Callable, *args, **kwargs):
        ...
        try:
            ...
            if hasattr(result, '__await__'):
                result = await result
            history_logger.debug(f'{self._get_task_name()} completing step {func_name} with {result}')
    ...
    async def record_external_event(self, name, identity, action, *args, **kwargs):
        ...
        history_logger.debug(f'External event {step_id} with {args} and {kwargs}')
    ...
    async def handle_internal_event(self, name, identity, action, *args, **kwargs):
        ...
        history_logger.debug(f'Calling {step_id} with {args} and {kwargs}')
        if inspect.iscoroutinefunction(function):
            result = await function(*args, **kwargs)
        else:
            result = function(*args, **kwargs)
    ...
    async def register_resource(self, name, identity, resource):
        ...
        history_logger.debug(f'Creating {resource_id}')
    ...
    async def delete_resource(self, name, identity, suspending=False):
        ...
        history_logger.debug(f'Removing {resource_id}')
    ...
    def start_task(self, func, *args, name=None, task_factory=asyncio.create_task, **kwargs):
        ...
        async def _func(*a, **kw):
            ...
            history_logger.debug(f'Completing task {task_id}')
    ...
    def signal_suspend(self):
        history_logger.debug(f'-- Suspending {self.workflow_id} --')
        ...
        for task in list(reversed(self._open_tasks)):
            if not task.done() or task.cancelled() or task.cancelling():
                history_logger.debug(f'Suspending task {task.get_name()}')
    ...
    async def suspend(self):
        ...
        for task in list(self._open_tasks):
            try:
                ...
                history_logger.debug(f'Task {task.get_name()} was cancelled')
                pass
