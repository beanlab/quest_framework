...
try:
    ...
    raise ImportError("The 'aws' extra is required to use this module. Run 'pip install history-py[aws]'.")
...
def create_s3_manager(
    ...
    def create_history(wid: str) -> Book:
        return PersistentList(wid, S3BlobStorage(wid, s3.get_s3_client(), s3.get_bucket_name()))

    return Historian(namespace, storage, create_history, factory)
...
class DynamoDB:
    def __init__(self):
        ...
        self._table_name = 'history_records'
...
def create_dynamodb_manager(
    ...
    def create_history(wid: str) -> Book:
        return PersistentList(wid, DynamoDBBlobStorage(wid, dynamodb.get_table()))
