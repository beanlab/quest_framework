...
from .history import History, find_history, SUSPENDED, wrap_methods_as_history_events
...
class InternalResource(Generic[T]):
    ...
    def __init__(self, name, identity, resource: T):
        ...
        self._historian = find_history()

    async def __aenter__(self) -> T:
        await self._historian.register_resource(self._name, self._identity, self._resource)
        return wrap_methods_as_history_events(self._resource, self._name, self._identity, self._historian)
...
class _ResourceWrapper:
    def __init__(self, name: str, identity: str | None, historian: 'History', resource_class):
        self._name = name
        self._identity = identity
...
def wrap_as_queue(name: str, identity: str | None, historian: History) -> Queue:
    return _ResourceWrapper(name, identity, historian, Queue)

def wrap_as_event(name: str, identity: str | None, historian: History) -> Event:
    return _ResourceWrapper(name, identity, historian, Event)

def wrap_as_state(name: str, identity: str | None, historian: History) -> State:
    return _ResourceWrapper(name, identity, historian, State)

def wrap_as_identity_queue(name: str, identity: str | None, historian: History) -> IdentityQueue:
