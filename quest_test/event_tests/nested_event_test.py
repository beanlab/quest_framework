...
async def test_nested_event(tmp_path):
    ...
    async with workflow_manager:
        ...
        assert result is None  # should be stopped by stop signal call
        ...
        assert 1 == result.payload["outer_event_count"]  # outer_event calls event_count once, should return 1
        assert 2 == result.payload["event_count"]  # event_count called two times, should return 2
        assert 0 == result.payload['self_event_counter']  # because it was replayed, self_event_count should be zero as all event_count calls should have been cached on the stop signals
    ...
    async with workflow_manager:
        ...
        assert 1 == result.payload["outer_event_count"]  # these three values should be the same as before we deserialized and reserialized
        assert 2 == result.payload["event_count"]
